...
class TeacherController extends Controller
{
  ...
  {
    $user = $teacher->user;
    return response()->json(['teacher' => $teacher, 'user' => $user, 'success' => true], 200);
  }
  ...
  public function getTeachers()
  {
    ...
    return response()->json(['teachers' => $teachers, 'success' => true], 200);
  }
  /**
  ...
  {
    if ($id && $cls_name = \App\User::find($id)->userable->class) {
      $cls = StudentClass::where('name', $cls_name)->get();
      // dd($cls[0]->id);
      if(is_integer($cls_name)){
        $cl_id  = $cls;
      }else{
        $cl_id = $cls[0]->id;
      }
      $list = Student::where('class_id', $cl_id)->get();
      // dd($list);
      return response()->json(['students' => $list, 'success' => true], 200);
    } else {
      return response()->json(['students' => [], 'success' => false], 404);
    }
  }
  public function postAttendance(Request $req)
  {
    $stds = $req->students; 
    foreach($stds as $st){
      Attendance::create(['student_id'=>$st, 'teacher_id'=>$req->teacher,'class'=>$req->class,'status'=>true]);
    }
    return response()->json(['students' => $stds, 'success' => true], 200);
  }
  public function getAttendance(Request $req)
  {
    $row = \DB::select("select * from attendance where teacher_id={$req->teacher_id} and class='{$req->class}' and date(created_at) in ('{$req->date}')");
    return response()->json(['students' => $row, 'success' => true], 200);
  }
}
